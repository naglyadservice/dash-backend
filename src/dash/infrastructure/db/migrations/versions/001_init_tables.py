...
def upgrade() -> None:
    ...
    sa.Enum("CARWASH", "WATER_VENDING", "VACUUM", name="transactiontype").create(
        op.get_bind()
    )
    sa.Enum(
        "BILL", "COIN", "PAYPASS", "MONOPAY", "LIQPAY", "FREE", name="paymenttype"
    ).create(op.get_bind())
    sa.Enum(
        "CREATED",
        "PROCESSING",
    ...
    sa.Enum("CARWASH", "WATER_VENDING", "VACUUM", name="controllertype").create(
        op.get_bind()
    )
    sa.Enum("SUPERADMIN", "COMPANY_OWNER", "LOCATION_ADMIN", name="adminrole").create(
        op.get_bind()
    )
    op.create_table(
        "admin_users",
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "company_id", "card_id", name="uix_customer_company_card_id"
        ),
        sa.UniqueConstraint("company_id", "email", name="uix_customer_company_email"),
    )
    op.create_index(
        "ix_customer_company_id", "customers", ["company_id", "id"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            postgresql.ENUM(
                "ACTIVE", "NOT_ACTIVE", name="controllerstatus", create_type=False
            ),
            nullable=False,
        ),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["controller_id"], ["controllers.id"], ondelete="SET NULL"
        ),
...
def downgrade() -> None:
    ...
    sa.Enum("SUPERADMIN", "COMPANY_OWNER", "LOCATION_ADMIN", name="adminrole").drop(
        op.get_bind()
    )
    sa.Enum("CARWASH", "WATER_VENDING", "VACUUM", name="controllertype").drop(
        op.get_bind()
    )
    ...
    sa.Enum("CARWASH", "WATER_VENDING", "VACUUM", name="transactiontype").drop(
        op.get_bind()
    )
    # ### end Alembic commands ###
