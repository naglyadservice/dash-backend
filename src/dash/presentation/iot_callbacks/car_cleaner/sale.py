...
from dash.services.iot.car_cleaner.dto import CarCleanerServiceEnum, CarCleanerRelayBit
...
@dataclass
class CarCleanerSaleCallbackPayload:
    id: int
    add_coin: int
    add_bill: int
    add_prev: int
...
car_cleaner_sale_callback_retort = Retort(
    recipe=[
        *datetime_recipe,
        name_mapping(
...
@tracer.wrap()
@parse_payload(retort=car_cleaner_sale_callback_retort)
@request_scope
@inject
async def car_cleaner_sale_callback(
    device_id: str,
    data: CarCleanerSaleCallbackPayload,
    ...
    dict_data = car_cleaner_sale_callback_retort.dump(data)
    controller = await controller_repository.get_by_device_id(device_id)

    if controller is None:
        logger.info(
            "CarCleaner sale request ignored: controller not found",
            device_id=device_id,
            data=dict_data,
        )
        return
    ...
    if (
        ...
        if customer is not None:
            ...
            logger.error(
                "CarCleaner sale request ignored: customer not found",
                device_id=device_id,
    ...
    transaction = CarCleanerTransaction(
        ...
        type=TransactionType.CAR_CLEANER,
    ...
    if not was_inserted:
        logger.info(
            "CarCleaner transaction was not inserted due to conflict",
            device_id=device_id,
            controller_id=controller.id,
            transaction_id=transaction.id,
            data=dict_data,
        )
        await car_cleaner_client.sale_ack(device_id, data.id)
        return
