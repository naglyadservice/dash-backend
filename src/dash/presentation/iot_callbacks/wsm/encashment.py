...
@dataclass
class WsmEncashmentCallbackPayload:
    id: int
    coin: list[int]
    bill: list[int]
    amount: int
...
wsm_encashment_callback_retort = Retort(recipe=[*datetime_recipe])


@tracer.wrap()
@parse_payload(retort=wsm_encashment_callback_retort)
@request_scope
@inject
async def wsm_encashment_callback(
    device_id: str,
    data: WsmEncashmentCallbackPayload,
    ...
    logger.info(
        "Wsm encashment request received",
        device_id=device_id,
        data=dict_data,
    )
