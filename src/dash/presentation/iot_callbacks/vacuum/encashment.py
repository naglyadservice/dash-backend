...
@dataclass
class VacuumEncashmentCallbackPayload:
    id: int
    coin: list[int]
    bill: list[int]
    amount: int
...
vacuum_encashment_callback_retort = Retort(recipe=[*datetime_recipe])


@tracer.wrap()
@parse_payload(retort=vacuum_encashment_callback_retort)
@request_scope
@inject
async def vacuum_encashment_callback(
    device_id: str,
    data: VacuumEncashmentCallbackPayload,
    ...
    dict_data = vacuum_encashment_callback_retort.dump(data)
    controller = await controller_repository.get_by_device_id(device_id)

    if controller is None:
        logger.info(
            "Vacuum encashment request ignored: controller not found",
            device_id=device_id,
            data=dict_data,
        )
        await vacuum_client.encashment_ack(
            device_id=device_id, payload={"id": data.id, "code": 1}
        )
        return

    logger.info(
        "Vacuum encashment request received",
        device_id=device_id,
        data=vacuum_encashment_callback_retort.dump(data),
    )
