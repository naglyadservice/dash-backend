...
async def laundry_state_info_callback(
    ...
    door = parse_state(cur_state, "input", controller.input_id)
    btn = parse_state(cur_state, "relay", controller.button_relay_id)
    led = parse_state(cur_state, "output", controller.led_output_id)
    ...
    logger.info(
        ...
        info={
            "door": "unlocked" if not door else "locked",
            "button": "unlocked" if btn else "locked",
